...
**Commands**
run AbilityExport		**Exports ALL abilities, including Tradeskills/Abilities/Spells/Combat Arts
...
function main(string Args)
{
	if ${Args.Length}==0
	{
		echo Using default settings: NT NP ADD (No Tradeskill, No Passive, Add to existing file). Use "Run AbilityExport help" for more options
		Args:Set[NT NP ADD]
	}
	if ${Args.Find[help]}
	{
		echo **Commands**
		echo run AbilityExport		**Exports ALL spells, including Tradeskills/Abilities/Spells/Combat Arts
	...
	;variable string ConfigFile="${Script.CurrentDirectory}/${Me.SubClass}_AbilityExport.xml"
	variable string ConfigFile="${Script.CurrentDirectory}/ability_lists/${Me.SubClass}_AbilityExport.xml"
