...
function:bool MoveCloser(float X, float Y, int Distance)
{
	...
	if ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}>${Distance}
	{
		...
		; Start moving
		;-------------------------------------------
		;echo "VG: Moving - ${Math.Distance[${Me.X},${Me.Y},${X},${Y}].Int} to ${Distance}"
		VG:ExecBinding[moveforward]

		;-------------------------------------------
		; Begin turning - nice cool effect
		;-------------------------------------------
		call faceloc ${X} ${Y} 20
		...
		;-------------------------------------------
		while ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}>${Distance} && ${LavishScript.RunningTime}<${bailOut} && !${Me.ToPawn.IsStunned} && !${isPaused}
		{
			VG:ExecBinding[moveforward]
			face ${X} ${Y}
		}
