...
function:bool MoveCloser(float X, float Y, int Distance=3)
{
	...
	if ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}<150 || ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}>${Distance}
	{
		...
		if ${X}==${Me.Target.X}
		{
			if ${Math.Distance[${Me.X},${Me.Y},${Me.Target.X},${Me.Target.Y}]}>${Distance} && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
			{
				CurrentAction:Set[MoveCloser - ${Me.Target.Name}]
				while ${Me.Target(exists)} && ${Math.Distance[${Me.X},${Me.Y},${Me.Target.X},${Me.Target.Y}]}>${Distance} && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
				{
			...
			if ${Math.Distance[${Me.X},${Me.Y},${Me.Target.X},${Me.Target.Y}]}<150 && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
			{
				CurrentAction:Set[MoveCloser - Backingup]
				while ${Me.Target(exists)} && ${Math.Distance[${Me.X},${Me.Y},${Me.Target.X},${Me.Target.Y}]}<150 && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
				{
					VG:ExecBinding[moveforward,release]
		...
		;-------------------------------------------
		if ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}>${Distance} && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
		{
			CurrentAction:Set[MoveCloser - (${Distance})]
			while !${Me.Target(exists)} && ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}>${Distance} && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
			{
				VG:ExecBinding[movebackward,release]
				VG:ExecBinding[moveforward]
				face ${X} ${Y}
		...
		;-------------------------------------------
		if ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}<150 && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
		{
			CurrentAction:Set[MoveCloser - Backingup]
			while ${Math.Distance[${Me.X},${Me.Y},${X},${Y}]}<150 && ${LavishScript.RunningTime}<=${bailOut} && !${Me.ToPawn.IsStunned} && !${Paused} && ${doMove}
			{
				VG:ExecBinding[moveforward,release]
				VG:ExecBinding[movebackward]
				face ${X} ${Y}
