...
function:bool faceslow(float facX, float facY, float Rt, float Lt)
{
	...
	{
		;-------------------------------------------
		; Modified:  Keep turning until we reached within 15 degrees of target or bailout timer runs out (2 sec)
		;-------------------------------------------
		while (${AbsAngle}>195 || ${AbsAngle}<165)  && (${LavishScript.RunningTime}<${bailOut})
		{
			;-------------------------------------------
...
function:float Angle(float angX, float angY)
{
	...
	;; Patch Angle if Dist>1 "Should fix the error - Nope but it was a good try"
