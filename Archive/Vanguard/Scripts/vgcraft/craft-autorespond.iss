...
function findAutoResponse(string aText, string aName)
{
	...
	call DebugOut "VG:AR: called with: ${aText}"

	setAutoRespond:GetSetIterator[anIter]
	if !${anIter:First(exists)}
	{
		call DebugOut "VG:AR: No auto-reponse values found in setAutoRespond"
		return
	}

	do
	{
		call DebugOut "VG:AR: Testing # ${anIter.Key}"
		...
		if ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[match](exists)}
		{
			...
			call DebugOut "VG:AR: test with aMatch: ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[match]}"
			...
			if ${aText.Find[${aMatch}]} || ${aMatch.Equal[ANY]}
			{
				...
				if  ${Time.Timestamp} > ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[time,0]}
				{
					...
					if !${isGMDetected} && ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[gmonly,FALSE]}
					{
						...
						call DebugOut "VG:AR: gmonly response"
						continue
					}
					...
					{
						call DebugOut "VG:AR: reply: ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[reply]}"
						if ${aName.Equal[NONE]}
					...
					{
						call DebugOut "VG:AR: emote: ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[emote]}"
					...
					{
						call DebugOut "VG:AR: say: ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[say]}"
					...
					{
						call DebugOut "VG:AR: action: ${setAutoRespond.FindSet[${anIter.Key}].FindSetting[action]}"
				...
				{
					call DebugOut "VG:AR: still waiting for timeout on: ${aMatch}"
				}
			}
		}
	}
...
function testSayEmoteText(string aText)
{
	...
	call DebugOut "VGAlarm: testing emote: ${aText}"
...
{
	call DebugOut "VGAlarm Tripped: (${Number}) :: ${aText}"
}
...
function AutoRespond(string aText, int Number)
{
	...
	if ${aText.Find[Server is shutting down]} && (${Number} == 0)
	{
		call TellsOut "${aText}"
