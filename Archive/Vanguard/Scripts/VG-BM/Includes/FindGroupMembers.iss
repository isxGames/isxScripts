...
function FindGroupMembers()
{
	...
	{
		echo "[${Time}][VG:EB] --> -------------------------------------"
		echo "[${Time}][VG:EB] --> FindGroupMembers: Finding Group Members and Tank"
	}
...
function CheckGroupMember(int GN)
{
	...
	{
		if ${doEcho} && ${GN}<7
		echo "[${Time}][VG:EB] --> FindGroupMembers: Group Member[${GN}]= ${Me.DTarget.Name}"
		if ${doEcho} && ${GN}>=7
		echo "[${Time}][VG:EB] --> FindGroupMembers: Tank= ${Me.DTarget.Name}"
	...
	{
		if ${doEcho} && ${GN}<7
		echo "[${Time}][VG:EB] --> FindGroupMembers: Group Member[${GN}]= does not exist"
		if ${doEcho} && ${GN}>=7
		echo "[${Time}][VG:EB] --> FindGroupMembers: Tank [${Tank}]= does not exist"
	}
}
