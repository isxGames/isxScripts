...
function Combat_Routine(int xAction)
{
	...
	;debuff check
	if (${DebuffMode} || ${EpicMode})
	{
		if ${Me.ToActor.Power}>30
		{
	...
	if ${Return} && ${DebuffCnt} > 0
		call CheckHeals

	;Start Wheel if Selected
	...
	;;;; FEAST
	;;
	if ${Actor[pc,exactname,${MainAssist}].Target(exists)} && ${Actor[pc,exactname,${MainAssist}].Target.Health}<30
	{
		call CheckCondition Power ${Power[${FeastAction},1]} ${Power[${FeastAction},2]}
		if ${Return.Equal[OK]}
			call CastSpellRange ${SpellRange[${FeastAction},1]} 0 0 0 ${Actor[pc,exactname,${MainAssist}].Target.ID}
	}
	...
	switch ${Action[${xAction}]}
	{
		...
		case AA_Thunderspike
			if (${MeleeAAAttacksMode} && ${Actor[${KillTarget}].Distance} <= 5) && ${OffenseMode}
			{
		...
		case AoE
			if ${AoEMode} && !${UseFastOffensiveSpellsOnly} && ${OffenseMode}
			{
				call CheckCondition Power ${Power[${xAction},1]} ${Power[${xAction},2]}
				if ${Return.Equal[OK]}
				{
		...
		case Proc
			if ${UseMeleeProcSpells} && ${Me.GroupCount}>1 && ${OffenseMode}
			{
				call CheckCondition Power ${Power[${xAction},1]} ${Power[${xAction},2]}
				if ${Return.Equal[OK]}
					call CastSpellRange ${SpellRange[${xAction},1]} 0 0 0 ${KillTarget}
		...
		case BallLightning
			if ${UseBallLightning} && ${OffenseMode}
			{
				call CheckCondition Power ${Power[${xAction},1]} ${Power[${xAction},2]}
				if ${Return.Equal[OK]}
				{
	    ...
	    if ${UseFastOffensiveSpellsOnly}
			...
			if ${Return.Equal[FALSE]} && (${OffenseMode} || ${DebuffMode})
			{
...
function CheckHeals()
{
  ...
  if ${Me.ID}!=${MainTankID} && ${Me.ToActor.Health}<50
	...
	if ${lowestset}
	{
		...
		{
			;echo "DEBUG:: ${Me.Group[${lowest}]}'s health is lowest (<50) at ${Me.Group[${lowest}].ToActor.Health} -- HEALING"
	...
	if (${MainTankExists})
	{
      ...
      {
    		if ${Me.Raid[${raidlowest}].ToActor.Health}<50 && !${Me.Raid[${raidlowest}].ToActor.IsDead} && ${Me.Raid[${raidlowest}].ToActor.Distance}<=${Me.Ability[${SpellType[2]}].Range}
    			call CastSpellRange 2 0 0 0 ${Me.Raid[${raidlowest}].ID}

    		if ${Me.Raid[${raidlowest}].ToActor.Health}<70 && !${Me.Raid[${raidlowest}].ToActor.IsDead} && ${Me.Raid[${raidlowest}].ToActor.Distance}<=${Me.Ability[${SpellType[1]}].Range}
    		{
    			...
    			elseif ${Me.Ability[${SpellType[7]}].IsReady}
    				call CastSpellRange 7 0 0 0 ${Me.Raid[${raidlowest}].ID}
    		}
    	}
  	}
  }
...
function HealMT(int MainTankID, int MTInMyGroup)
{
	...
	if ${Actor[${MainTankID}].Health}<90 && ${Actor[${MainTankID}](exists)} && !${Actor[${MainTankID}].IsDead}
	{
		...
		elseif ${Me.Ability[${SpellType[7]}].IsReady} && ${Me.Maintained[${SpellType[7]}].Target.ID} != ${Actor[PC,ExactName,${MainTankPC}].ID} && ${Actor[${MainTankID}].Distance}<=${Me.Ability[${SpellType[7]}].Range}
...
function GroupHeal()
{
	...
	if ${Me.Ability[${SpellType[15]}].IsReady} && !${Me.Maintained[${SpellType[11]}](exists)}
		call CastSpellRange 15
	else
...
function CureGroupMember(int gMember)
{
	...
	while ${Me.Group[${gMember}].IsAfflicted} && ${tmpcure:Inc}<3 && ${Me.Group[${gMember}].ToActor(exists)} && !${Me.Group[${gMember}].ToActor.IsDead}
	{
...
function CheckHOTs()
{
	...
	if (${Me.InCombat} && ${KeepGroupHOTUp}) || ${KeepReactiveUp} && !${Me.Maintained[${SpellType[11]}](exists)}
		call CastSpellRange 11
