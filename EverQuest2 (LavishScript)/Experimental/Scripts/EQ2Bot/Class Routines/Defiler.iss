...
function CheckCures()
{
	...
	do
	{
		...
		if ${Me.ToActor.Health}<50 && !${EpicMode}
			call HealMe
	}
	...
	while ${Me.ToActor.Health}>50 && !${Me.IsAfflicted}
}
...
function CheckHeals()
{
	...
	do
	{
		if ${Me.Group[${temphl}].ToActor(exists)}
		{
			...
			if (${Me.Group[${temphl}].Class.Equal[conjuror]}  || ${Me.Group[${temphl}].Class.Equal[necromancer]}) && if ${Me.Group[${temphl}].ToActor.Pet.Health}<60 && ${Me.Group[${temphl}].ToActor.Pet.Health}>0
	...
	if ${PetMode} && ${grpheal} && ${Me.Ability[${SpellType[220]}].IsReady}
...
function HealMT(int MainTankID)
{
	;DeathWard Check
