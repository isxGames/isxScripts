...
function Combat_Routine(int xAction)
{
	...
	{
		;Glacial Shield - DS
		if ${Me.Ability[${SpellType[355]}].IsReady} && !${Me.Maintained[${SpellType[355]}](exists)} && ${spellsused}
		{
			call CastSpellRange 355
		}

		;Ice Spear - Cold Dot + Debuff
		if ${Me.Ability[${SpellType[73]}].IsReady} && !${Me.Maintained[${SpellType[73]}](exists)} && ${spellsused}
		{
			call CastSpellRange 73
		}

		;Storming Tempest - Magic Dot
		if ${Me.Ability[${SpellType[70]}].IsReady} && !${Me.Maintained[${SpellType[70]}](exists)} && ${spellsused}
		{
			call CastSpellRange 70
		}

		;Ice Nova/Bolt of Ice + Freehand Scorcery
		...
		;deaggro
		if ${Me.Ability[${SpellType[180]}].IsReady} && !${Me.Maintained[${SpellType[180]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 180
		}

		;Surge of Ro - Dot Proc
		if ${Me.Ability[${SpellType[361]}].IsReady} && !${Me.Maintained[${SpellType[361]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 361
		}

		;ProtoFire Pet - Pet
		if ${Me.Ability[${SpellType[324]}].IsReady} && !${Me.Maintained[${SpellType[324]}](exists)} && ${PetMode} && ${spellsused}<4
		{
			call CastSpellRange 324
		}

		;Velium Gift + Iceshape
		if ${Me.Ability[${SpellType[360]}].IsReady} && ${spellsused}<4
		{
			if ${Me.Ability[${SpellType[388]}].IsReady}
			{
				call CastSpellRange 388
			}
			call CastSpellRange 360 0 0 0 ${KillTarget}
		}

		;Ball of Magma - Heat Nuke
		if ${Me.Ability[${SpellType[62]}].IsReady} && !${Me.Maintained[${SpellType[62]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 62
		}	

		;Magma Chamber - Heat Nuke
		if ${Me.Ability[${SpellType[181]}].IsReady} && !${Me.Maintained[${SpellType[181]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 181
		}

		;Flames of Velious - Colf/Heat Nuke 
		if ${Me.Ability[${SpellType[61]}].IsReady} && !${Me.Maintained[${SpellType[61]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 61
		}

		;HeatWave - Heat Nuke
		if ${Me.Ability[${SpellType[71]}].IsReady} && !${Me.Maintained[${SpellType[71]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 71
		}

		;Ro's Coil - Heat Dot
		if ${Me.Ability[${SpellType[72]}].IsReady} && !${Me.Maintained[${SpellType[72]}](exists)} && ${spellsused}<4
		{
			call CastSpellRange 72
		}
	}
