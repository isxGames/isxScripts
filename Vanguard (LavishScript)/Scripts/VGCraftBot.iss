...
variable string savePath = "${Script.CurrentDirectory}/vgcraft/save"
variable filepath VGPathsDir = "${Script.CurrentDirectory}/vgcraft/vgpaths/"
...
{
	echo "VG: Updated of VGCraftBot all done!"
...
function main(int testParam)
{
	...
	variable string noUpdateFile = VGCraftBot.iss.nopatch
	...
	else
	{
		...
		if ( ${testParam} == -1 )
		{
			...
			dotnet VGCraftBot isxGamesPatcher VGCraftBot ${VERSION} http://www.reality.net/svn/vanguard/VGCraftBot-devtree.xml
		}
		else
		{
			; Run the auto-update patcher for Release tree
			echo "VG: Running patch checker for Updates"
			dotnet VGCraftBot isxGamesPatcher VGCraftBot ${VERSION} http://www.reality.net/svn/vanguard/release/VGCraftBot-manifest.xml
		}
	...
	if ( ${autoUpdateFile} )
	{
		echo "VGCraftBot updated, exiting. Please runscript again"
		return
	}
	*/
	...
	if !${SkinPath.FileExists[VGSkin.xml]}
	{
		...
		dotnet VGSkin "${Script.CurrentDirectory}/vgcraft/VGSkin.exe"
	...
	*/
	...
	else
	{
		...
		variable filepath fSavePath = "${Script.CurrentDirectory}/vgcraft/save/${Me.FName}/"
		...
		{
			echo "VG: NOTICE: Could not find ${fSavePath.AbsolutePath} autorespond.xml"
		}
		...
		do 
		{
			...
			if ( ${Me.CraftXP} > ${lastCraftXP} )
			{
				...
				timeCheck:Set[${Math.Calc[${Script[VGCraftBot].RunningTime}/1000/60/60]}]
...
function atexit() 
{
	...
	totalTime:Set[${Math.Calc[(${Script[VGCraftBot].RunningTime}/1000/60/60)%60].Int.LeadingZeroes[2]}:${Math.Calc[(${Script[VGCraftBot].RunningTime}/1000/60)%60].Int.LeadingZeroes[2]}:${Math.Calc[(${Script[VGCraftBot].RunningTime}/1000)%60].Int.LeadingZeroes[2]}]
